...
async function run() {
  ...
  try {
    const root = (0, import_node_child_process.execSync)(`git merge-base ${base} HEAD`).toString().trim();
    const target = (0, import_node_child_process.execSync)(`git log -n 1 --pretty=format:"%H"`).toString().trim();
    const stdout = (0, import_node_child_process.execSync)(
      `git diff --word-diff --unified=0 ${root} ${target} -- ${path} | sed -nr -e "s${MOD_PATTERN}M/p" -e "s${ADD_PATTERN}A/p" -e"s${REM_PATTERN}R/p" | sort | uniq -c`
    )
      .toString()
      .trim();
    core.debug("root: " + root);
    core.debug("target: " + target);
    core.debug("diff: " + stdout);
    const added = parseInt(ADDED.exec(stdout)?.[0].split(" ")[0] ?? "0", 10);
    const modified = parseInt(MODIFIED.exec(stdout)?.[0].split(" ")[0] ?? "0", 10);
    const removed = parseInt(REMOVED.exec(stdout)?.[0].split(" ")[0] ?? "0", 10);
